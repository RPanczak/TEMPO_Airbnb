...
SA3 <- readRDS(file = "./data/geo/SA3_2016_AUST_clean.rds") %>% 
  ...
  select(-starts_with("SA3"), -starts_with("GCC"), -AREASQKM16)
...
SA3 %<>% left_join(select(airbnb_sa3_18_wide, SA3_CODE16, starts_with("cluster"))) %>% 
  filter(SA3_NAME16 != "Illawarra Catchment Reserve")
...
# tabyl(airbnb_sa3, STE_NAME16)
# tabyl(airbnb_sa3, State_short)
```
