...
APM_sa2 %<>% 
  group_by(SA2_MAIN16, property_type) %>% 
  mutate(record_count_int = as.integer(round(na_interpolation(record_count, option = "stine"))),
...
APM_sa2 %>% 
  ...
  ggplot(aes(x = date, y = record_count, group = date)) + 
  geom_boxplot() + 
...
Large chunks of records is labelled as not active. 

```{r}
frq(monthly_new_prep_revenue, active, sort.frq = "desc") 
```
