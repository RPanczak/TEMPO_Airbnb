...
# joins
ha_SA1_2016_join <- read_csv("./data/ha/raw/ha_SA1_2016_join.zip") %>% 
  dplyr::rename(property_id = PROPERTY_ID) %>% 
  select(-XCoord, -YCoord)

saveRDS(ha_SA1_2016_join, file = "./data/ha/clean/ha_SA1_2016_join.rds")

ha_POA_2016_join <- read_csv("./data/ha/raw/ha_POA_2016_join.zip") %>% 
  dplyr::rename(property_id = PROPERTY_ID) %>% 
  select(-XCoord, -YCoord)

saveRDS(ha_POA_2016_join, file = "./data/ha/clean/ha_POA_2016_joinn.rds")
```
...
Separate preparation is done for `ha` (properties) and `ham` (monthly) datasets. 
...
```{r, include=FALSE}
ha_raw <- read_csv("./data/ha/raw/Australia_HA_Property_2019-02-01.zip")
...
saveRDS(hap, "./data/ha/clean/hap.Rds")
```
...
# write_csv(ha_select, "./data/ha/clean/hap_select.csv")
saveRDS(hap_select, "./data/ha/clean/hap_select.Rds")
...
```{r, include=FALSE}
ham_raw <- read_csv("data/ha/raw/Australia_HA_Monthly_2019-02-01.zip", 
...
saveRDS(ham, "./data/ha/clean/ham.Rds")
```
...
```{r}
ham_select <- ham %>% 
  select(property_id, active, reporting_month, 
         number_of_reservations, occupancy_rate, 
         reservation_days, available_days, blocked_days,
         revenue_usd, adr_usd)

# write_csv(ham_select, "./data/ha/clean/ham_select.csv")
saveRDS(ham_select, "./data/ha/clean/ham_select.Rds")
