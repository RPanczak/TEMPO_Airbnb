...
Raw datasets consist of `r comma(length(unique(monthly_select$property_id)))` properties and 
`r comma(nrow(monthly_select))` monthly stats. 

## Selection

`r comma(nrow(filter(monthly_select, reporting_month < ymd("2016-01-01"))))` data points from before 2016-01-01 were excluded.
...
Final dataset consists of `r comma(nrow(monthly_select))`
monthly records of `r comma(length(unique(monthly_select$property_id)))` properties. 
...
`r nrow(filter(SA2, !SA2_MAIN16 %in% unique(monthly_select$SA2_MAIN16)))` SA2s (out of `r nrow(SA2)`) have no Airbnbs inside. 

```{r}
temp <- SA2 %>% 
  filter(!SA2_MAIN16 %in% unique(monthly_select$SA2_MAIN16))
...
stopifnot(nrow(SA2) * length(unique(monthly_select$reporting_month)) == nrow(airbnb_sa2))

# table(is.na(monthly_select$SA1_7DIG16))
...
airbnb_HOSD_mis <- monthly_select %>%
...
# checking for airbnbs
airbnb_SEIFA_mis <- monthly_select %>%
...
# checking for airbnbs
airbnb_SEIFA_na <- monthly_select %>%
