...
rm(zero_res, OT, OT_monthly, zero_days, monthly_select, property_select, temp)
gc()
```
...
Only SA3s that had all SEIFA indices calculated were included in the analyses.
...
airbnb_sa3 <- left_join(st_drop_geometry(SA3), temp) %>% 
  ...
  group_by(SA3_CODE16) %>% arrange(reporting_month) %>% mutate(cumulative = cumsum(reservations)) %>% 
  ungroup() %>% 
...
'Growth curves' with `r nrow(airbnb_sa3)` data points of cumulative Airbnb revenue for `r nrow(SA2)` areas:

```{r}
ggplot(data = airbnb_sa3, 
       aes(x = reporting_month, y = cumulative/1000, 
           color=STE_NAME16, group=SA3_CODE16)) +
  geom_line(alpha=0.5) +
...
airbnb_sa3_18 <- airbnb_sa3 %>% 
  ...
  group_by(SA3_CODE16) %>% arrange(reporting_month) %>% mutate(yearly = sum(reservations)) %>% 
  ungroup() %>% 
  mutate(share = (reservations / yearly) *100) %>% 
  arrange(SA3_CODE16, reporting_month)
```
...
ggplot(data = airbnb_sa3_18, 
  ...
  geom_line(alpha=0.5) +
  theme_minimal() + xlab("") + ylab("Airbnb reservations [*1,000]") +
  facet_grid(~STE_NAME16)
```

### Relative 2018

```{r}
# sjmisc::descr(airbnb_sa3_18, share)
# ggplot(airbnb_sa3_18, aes(share)) + geom_histogram()
# sjmisc::descr(airbnb_sa3_18, yearly)
# ggplot(airbnb_sa3_18, aes(yearly)) + geom_histogram()

ggplot(data = airbnb_sa3_18, 
       aes(x = reporting_month, y = share, group=SA3_CODE16, colour = sqrt(yearly))) +
  geom_line(alpha=0.5) +
