...
Raw dataset consists of `r format(nrow(monthly_select), nsmall=0, big.mark=",")` monthly stats of `r format(length(unique(monthly_select$property_id)), nsmall=0, big.mark=",")` properties. 
...
Final dataset of `r format(nrow(monthly_new_prep_gno), nsmall=0, big.mark=",")`
monthly records of `r format(length(unique(monthly_new_prep_gno$property_id)), nsmall=0, big.mark=",")` properties was aggregated to SA3 levels. 
...
```{r}
SA3_na <- anti_join(SA3, select(monthly_new_prep_gno, SA3_CODE16))
```
