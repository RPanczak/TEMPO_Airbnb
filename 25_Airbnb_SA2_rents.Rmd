...
monthly records of `r format(length(unique(airbnb_new_monthly$property_id)), nsmall=0, big.mark=",")` properties was aggregated to SA2 levels. 
...
`r nrow(filter(SA2, !SA2_MAIN16 %in% unique(airbnb_new_monthly$SA2_MAIN16)))` areas out of `r nrow(SA2)` SA2s included have no Airbnbs inside. 

```{r}
temp <- SA2 %>% filter(!SA2_MAIN16 %in% unique(airbnb_new_monthly$SA2_MAIN16))
...
`r nrow(filter(SA2, SA2_MAIN16 %in% unique(airbnb_new_monthly$SA2_MAIN16)))` areas out of `r nrow(SA2)` SA2s included have Airbnbs inside. 
