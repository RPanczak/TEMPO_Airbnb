...
APM_sa2 %>% 
  ...
  ggplot(aes(x = date, y = adr_50p_int*7)) + 
...
APM_sa2 %>% 
  ...
  ggplot(aes(x = adr_50p_int*7, y = rent_50p_int)) + 
...
![](plot/apm/adr_1/Arncliffe - Bardwell Valley.png)
...
for (i in 1:length(temp)) {
  ...
  if (nrow(label_house) > 0) {
    ...
    bind_rows(
      ...
      overlap %>%
        filter(SA2_NAME16 == temp[i]) %>% 
        filter(property_type == "House") %>% 
        select(date, adr_25p_int, adr_50p_int, adr_75p_int, n_listings) %>% 
        rename(p25 = adr_25p_int, p50 = adr_50p_int, 
               p75 = adr_75p_int, n = n_listings) %>% 
        mutate(type = "Airbnb") 
    ...
    path <- paste0("plot/apm/adr_2/", temp[i], " - house", ".png")
  ...
  if (nrow(label_unit) > 0) {
    ...
    path <- paste0("plot/apm/adr_2/", temp[i], " - unit", ".png")
...
![](plot/apm/adr_2/Arncliffe - Bardwell Valley - house.png)

![](plot/apm/adr_2/Arncliffe - Bardwell Valley - unit.png)
