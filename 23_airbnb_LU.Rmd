...
airbnb_poa_monthly_clusters %>% 
  ...
  filter(row_number() == 1) %>% 
  ungroup() %>% 
  ggplot(aes(x= Cluster, y = month_gno_tot)) + 
...
```{r}
airbnb_poa_monthly_clusters %>% 
  ggplot(aes(x = reporting_month, y = month_gno_share, group = POA_CODE16)) + 
  geom_line(colour = "purple4", alpha = .33) +
  facet_wrap(~Cluster) +
  theme_light()
```
...
```{r}
airbnb_poa_monthly_clusters %>% 
  ggplot(aes(x = reporting_month, y = month_gno_share, group = reporting_month)) + 
  geom_boxplot(varwidth = TRUE) +
  facet_wrap(~Cluster) +
  theme_light()
```
