...
# long format
supply <- bind_rows(
  SA2_U %>% rename(stock = units) %>% mutate(property_type_new = "Apartment"),
...
##### Total properties in region

Apartment

```{r}
ggplot(data = filter(airbnb_sa2_supply_n, property_type_new == "Apartment"),
...
```{r}
ggplot(data = filter(airbnb_sa2_supply_days, property_type_new == "Apartment"),
...
This is a bit more elaborate indicator that takes into account all active properties listed in given month in SA2 and uses their `reservation_days` as measure. 
...
```{r}
ggplot(data = filter(airbnb_sa2_demand_days, property_type_new == "Apartment"),
...
Each property has a 'trajectory' of income data.  Not all months are available.. Not all months have income.. 
...
```{r}
ggplot(data = filter(airbnb_sa2_revenue, property_type_new == "Apartment"),
