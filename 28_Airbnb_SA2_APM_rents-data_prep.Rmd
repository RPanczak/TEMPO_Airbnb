...
temp <- APM_sa2 %>% 
  ...
  ungroup() %>% 
  mutate(stock_difference = ifelse(property_type == "Unit", 
                                   max_record_count - units, max_record_count - houses)) %>% 
  group_by(SA2_MAIN16, property_type) %>% 
  filter(row_number() == 1) %>% 
  ungroup() %>% 
  filter(stock_difference > 0) %>% 
  arrange(property_type, stock_difference) %>% 
...
`r comma(nrow(OT))` records of `r comma(length(unique(OT$property_id)))` properties located in remote areas were excluded. 
...
`r comma(nrow(filter(monthly_new_prep_rent, is.na(property_type_new))))` ambiguous/imprecise
monthly records of `r comma(nrow(filter(monthly_new_prep_rent, is.na(property_type_new)) %>% select(property_id) %>% distinct()))` properties were excluded

```{r include=FALSE}
monthly_new_prep_rent %<>% 
  filter(! is.na(property_type_new)) %>%
...
```{r}
frq(monthly_new_prep_rent, property_type_new, sort.frq = "desc") 
```
...
temp %>%
  tabyl(bedrooms, property_type_new) %>%
...
So the number of Airbnbs and income from them will be later fixed to zero in that locations. 
...
# long format
supply <- bind_rows(
  SA2_U %>% rename(stock = units) %>% mutate(property_type_new = "Unit"),
  SA2_H %>% rename(stock = houses) %>% mutate(property_type_new = "House")
) %>% 
  select(SA2_NAME16, property_type_new, stock) %>% 
  arrange(SA2_NAME16, property_type_new)
...
Note that this is 'static' variable without temporal variation.

#### Binary

This is *naive* indicator that takes into account all **active** properties listed in given month in SA2 irrespectively if they were used or not.
...
monthly_new_prep_rent %>% 
  ...
  geom_col() +
  facet_wrap(vars(property_type_new)) +
...
**Areas with zero stocks of census properties and any number of propoerties had this measure fixed to number of propoerties.** That for the moment produces high values in certain areas.

```{r, include=FALSE}
temp <- monthly_new_prep_rent %>% 
  group_by(SA2_MAIN16, property_type_new, reporting_month) %>% 
  summarise(supply_n = n())

airbnb_sa2_supply_n <- left_join(st_drop_geometry(SA2), temp) %>% 
  expand(SA2_MAIN16, property_type_new, reporting_month) %>% 
  ...
  ungroup() %>% 
  arrange(SA2_MAIN16, property_type_new, reporting_month)
...
airbnb_sa2_supply_n %<>% 
  left_join(supply) %>%   
  mutate(stock = as.integer(stock)) %>% 
  mutate(supply_n_rel = supply_n/stock) %>% 
  mutate(supply_n_rel = ifelse(supply_n == 0 & stock == 0, 0, supply_n_rel)) %>% 
  mutate(supply_n_rel = ifelse(supply_n > 0 & stock == 0, supply_n, supply_n_rel))
...
```{r}
ggplot(data = filter(airbnb_sa2_supply_n, property_type_new == "Unit"),
       aes(x=reporting_month, y=supply_n, 
           group=SA2_MAIN16)) +
  geom_line(col = "darkorchid4", alpha = .5) + 
  facet_wrap(vars(STE_NAME16)) +
  theme_minimal() + xlab("") + ylab("Total Airbnb properties")
```

House

```{r}
ggplot(data = filter(airbnb_sa2_supply_n, property_type_new == "House"),
       aes(x=reporting_month, y=supply_n, 
           group=SA2_MAIN16)) +
  geom_line(col = "darkorchid4", alpha = .5) + 
  facet_wrap(vars(STE_NAME16)) +
...
SA2_centr %>% 
  ...
  tm_dots(size = "supply_n", col = "supply_n_rel", palette = "YlOrRd", n = 7, 
          id = "SA2_NAME16", popup.vars = c("SA2_NAME16", "supply_n", "stock", "supply_n_rel")) +
  tm_facets(by = "property_type_new", free.scales = FALSE, sync = TRUE)
```
...
monthly_new_prep_rent %>% 
  ...
  geom_col() +
  facet_wrap(vars(property_type_new)) +
  theme_minimal() + xlab("") + ylab("Total available days")
```
...
**Areas with zero stocks of census properties and any number of propoerties had this measure fixed to number of propoerties * `available_days`.** That for the moment produces high values in certain areas.
...
# summarytools::freq(airbnb_sa2_supply_days$property_type_new)
...
airbnb_sa2_supply_days %<>% 
  left_join(supply) %>%   
  mutate(stock = as.integer(stock)) %>% 
  mutate(stock = stock * days_in_month(reporting_month)) %>% 
  mutate(supply_days_rel = supply_days / stock) %>% 
  mutate(supply_days_rel = ifelse(supply_days == 0 & stock == 0, 0, supply_days_rel)) %>% 
  mutate(supply_days_rel = ifelse(supply_days > 0 & stock == 0, supply_days, supply_days_rel))
...
```{r}
ggplot(data = filter(airbnb_sa2_supply_days, property_type_new == "Unit"),
...
```{r}
ggplot(data = filter(airbnb_sa2_supply_days, property_type_new == "House"),
       aes(x=reporting_month, y=supply_days, 
           group=SA2_MAIN16)) +
  geom_line(col = "darkorchid4", alpha = .5) + 
  facet_wrap(vars(STE_NAME16)) +
...
SA2_centr %>% 
  ...
  tm_dots(size = "supply_days", col = "supply_days_rel", palette = "YlOrRd", n = 7, 
          id = "SA2_NAME16", popup.vars = c("SA2_NAME16", "supply_days", "stock", "supply_days_rel")) +
  tm_facets(by = "property_type_new", free.scales = FALSE, sync = TRUE)
```
...
**Areas with zero stocks of census properties and any number of propoerties had this measure fixed to number of propoerties * `reservation_days`.** That for the moment produces high values in certain areas.
...
airbnb_sa2_demand_days %<>% 
  ...
  mutate(stock = as.integer(stock)) %>% 
  mutate(stock = stock * days_in_month(reporting_month)) %>% 
  mutate(demand_days_rel = demand_days / stock) %>% 
  mutate(demand_days_rel = ifelse(demand_days == 0 & stock == 0, 0, demand_days_rel)) %>% 
  mutate(demand_days_rel = ifelse(demand_days > 0 & stock == 0, demand_days, demand_days_rel))
...
```{r}
ggplot(data = filter(airbnb_sa2_demand_days, property_type_new == "Unit"),
...
```{r}
ggplot(data = filter(airbnb_sa2_demand_days, property_type_new == "House"),
       aes(x=reporting_month, y=demand_days, 
           group=SA2_MAIN16)) +
  geom_line(col = "darkorchid4", alpha = .5) + 
  facet_wrap(vars(STE_NAME16)) +
...
SA2_centr %>% 
  ...
  tm_dots(size = "demand_days", col = "demand_days_rel", palette = "YlOrRd", n = 7, 
          id = "SA2_NAME16", popup.vars = c("SA2_NAME16", "demand_days", "stock", "demand_days_rel")) +
  tm_facets(by = "property_type_new", free.scales = FALSE, sync = TRUE)
```
...
```{r}
ggplot(data = filter(airbnb_sa2_revenue, property_type_new == "Unit"),
...
```{r}
ggplot(data = filter(airbnb_sa2_revenue, property_type_new == "House"),
       aes(x=reporting_month, y=revenue_tot/1000000, 
           group=SA2_MAIN16)) +
  geom_line(col = "darkorchid4", alpha = .5) + 
  facet_wrap(vars(STE_NAME16)) +
...
SA2_centr %>% 
  ...
  filter(property_type_new == "House") %>% 
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
...
```{r}
# !!!
# airbnb_sa2_revenue %<>% left_join(select(SEIFA, SA2_MAIN16, ends_with("_d")))
```
...
airbnb_sa2_revenue %<>% left_join(coast_bin_sa2)
...
```{r}
filter(airbnb_sa2_revenue, reporting_month == as.Date("2017-10-01")) %>% 
  frq(coast_bin)
```
...
```{r}
saveRDS(airbnb_sa2_revenue, file = "./data/airdna/clean/airbnb_sa2_revenue.rds")
# write_csv(airbnb_sa2_revenue, path = "./data/airdna/clean/airbnb_sa2_revenue.csv", 
