...
Starting point is set of `r number(nrow(SA2))` areas. This excludes 4 remote areas - three from Other Territories (but keeps Jervis Bay!) and one from NSW (Lord Howe Island)

```{r}
# Christmas Island, Cocos (Keeling) Islands, Norfolk Island, Lord Howe Island
...
# 
# readRDS(file = "./data/geo/SA2_2016_AUST_clean.rds") %>% 
#   filter(SA2_NAME16 == "Norfolk Island") %>% 
#   # select(SA2_NAME16, STE_NAME16) %>% 
#   st_drop_geometry()
```
...
summary(monthly_new_select$reporting_month)
```
...
```{r include=FALSE}
saveRDS(monthly_new_prep_revenue, "./data/airdna/clean/monthly_new_prep_revenue.rds")

monthly_new_prep_revenue <- readRDS("./data/airdna/clean/monthly_new_prep_revenue.rds")
```
