...
subtitle: "Monthly relative income on SA4_GCC level"
...
Prepared dataset of `r format(nrow(airbnb_monthly), nsmall=0, big.mark=",")`
...
stopifnot(nrow(SA4_GCC) * length(unique(airbnb_monthly$reporting_month)) == nrow(airbnb_sa4))
rm(temp, airbnb_monthly)
gc()
```
...
airbnb_sa4 %>% 
  ...
  select(SA4_GCC_NAME16, reporting_month)
```
...
ggplot(airbnb_sa4_12mo, 
       aes(x=reporting_month, y=revenue_rel)) +
  geom_line(aes(color=cluster3, group=SA4_GCC_CODE16), alpha=0.35) +
...
tabyl(SA4_GCC, cluster3)

tm_shape(SA4_GCC) +
  tm_polygons("cluster3", border.col = "white", lwd = 0.5, 
              id = "SA4_GCC_NAME16", popup.vars = c("cluster3"), 
              palette = "Dark2") +
  tm_shape(STE) +
...
ggplot(airbnb_sa4_12mo, 
       aes(x=reporting_month, y=revenue_rel)) +
  geom_line(aes(color=cluster5, group=SA4_GCC_CODE16), alpha=0.35) +
...
tabyl(SA4_GCC, cluster5)

tm_shape(SA4_GCC) +
  tm_polygons("cluster5", border.col = "white", lwd = 0.5, 
              id = "SA4_GCC_NAME16", popup.vars = c("cluster5"), 
              palette = "Dark2") +
  tm_shape(STE) +
  tm_borders(lwd = 0.5, alpha = 0.5)  
```
