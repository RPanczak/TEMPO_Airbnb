  ...
  if (rolling == 0) {
    ...
    temp <- monthly_new_prep_rent %>% 
      ...
      summarise(
        ...
        revenue_med = median(revenue_native, na.rm = TRUE),
        adr_p25 = quantile(adr_native, probs = .25, na.rm = TRUE),
        adr_med = median(adr_native, na.rm = TRUE),
        adr_p75 = quantile(adr_native, probs = .75, na.rm = TRUE)) %>% 
      ungroup() %>% 
...
airbnb_sa2_revenue <- left_join(st_drop_geometry(SA2), temp) %>% 
  ...
  mutate(adr_med = as.integer(round(adr_med))) %>%
  mutate_if(is.factor, as.character) %>% 
  ungroup() %>% 
...
APM_sa2 %<>%
  ...
  left_join(
    ...
    select(airbnb_sa2_revenue, 
           ...
           n_obs, n_months, revenue_med, adr_p25, adr_med, adr_p75)
  ) %>% 
  arrange(SA2_MAIN16, property_type, date)
...
APM_sa2 %>%
  ...
  ggplot(aes(x=date)) +
  geom_line(aes(y=rent_50p_int), size = 1) +
...
APM_sa2 %>%
  filter(SA2_NAME16 %in% c("Byron Bay")) %>% 
  ggplot(aes(x=date)) +
  geom_line(aes(y=adr_med), size = 1) +
  geom_point(aes(y=adr_med, size = n_obs)) + 
  facet_wrap(~property_type) +
  theme_minimal() + xlab("") + ylab("Monthly revenue AUD")
```
