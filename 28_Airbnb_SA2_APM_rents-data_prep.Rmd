...
```{r warning=FALSE, fig.width=6, fig.height=3, out.width="600px", out.height="300px"}
APM_sa2 %>% 
...
```{r warning=FALSE}
# listings without full time series
# monthly_new_prep_rent %>% 
#   group_by(property_id) %>% 
#   summarise(n = n()) %>% 
...
monthly_new_prep_rent %>% 
  ...
  geom_line(aes(group=property_id), size = 1) +
  geom_point() + 
  theme_minimal() + xlab("") + ylab("Monthly ADR AUD")
```
...
#### Averages from last 12 months

```{r, include=FALSE}
# simply groupping by monthh
# temp <- monthly_new_prep_rent %>% 
...
#             adr_50p = median(adr_native, na.rm = TRUE))

dates_sequence <- seq(as.Date("2016-01-01"), by = "month", length.out = 12)

# dates_min %m+% months(0)
# dates_max %m+% months(0)
# 
# dates_min %m+% months(26)
# dates_max %m+% months(26)

dates_min <- min(dates_sequence)
dates_max <- max(dates_sequence)

for (rolling in 0:26){
  
  print(paste("Using data from:", dates_min, "to:", dates_max))
  
  if (rolling == 0) {
    
    temp <- monthly_new_prep_rent %>% 
      filter(reporting_month >= dates_min & reporting_month <= dates_max) %>% 
      group_by(SA2_MAIN16, property_type) %>% 
      summarise(
        # n_obs = n(),
        n_listings = n_distinct(property_id),
        n_months = n_distinct(reporting_month),
        # revenue_tot = sum(revenue_native, na.rm = TRUE),
        # revenue_med = median(revenue_native, na.rm = TRUE),
        adr_25p = quantile(adr_native, probs = .25, na.rm = TRUE),
        adr_50p = quantile(adr_native, probs = .50, na.rm = TRUE),
        adr_75p = quantile(adr_native, probs = .75, na.rm = TRUE)) %>% 
      ungroup() %>% 
      mutate(date = dates_max)
    
  } else {
    
    add <- monthly_new_prep_rent %>% 
      filter(reporting_month >= dates_min & reporting_month <= dates_max) %>% 
      group_by(SA2_MAIN16, property_type) %>% 
      summarise(
        # n_obs = n(),
    ...
    temp <- bind_rows(temp, add)
    
  }
  
  dates_min <- dates_min %m+% months(1)
  dates_max <- dates_max %m+% months(1)  
...
airbnb_sa2_revenue <- left_join(st_drop_geometry(SA2), temp) %>% 
  expand(SA2_MAIN16, property_type, date) %>% 
  filter(!is.na(date)) %>% 
  filter(!is.na(property_type)) %>% 
  ...
  # tidyr::replace_na(list(n_obs = 0)) %>% 
  tidyr::replace_na(list(n_listings = 0)) %>% 
  tidyr::replace_na(list(n_months = 0)) %>% 
  # tidyr::replace_na(list(adr_25p = 0)) %>% 
  # tidyr::replace_na(list(adr_50p = 0)) %>% 
  # tidyr::replace_na(list(adr_75p = 0)) %>% 
  # mutate(revenue_tot = as.integer(round(revenue_tot))) %>% 
  # mutate(revenue_med = as.integer(round(revenue_med))) %>%
  # mutate(n_obs = as.integer(round(n_obs))) %>%
  mutate(n_listings = as.integer(round(n_listings))) %>%
  mutate(n_months = as.integer(round(n_months))) %>%
  # mutate(adr_25p = as.integer(round(adr_25p))) %>%
  # mutate(adr_50p = as.integer(round(adr_50p))) %>%
  # mutate(adr_75p = as.integer(round(adr_75p))) %>%
  mutate_if(is.factor, as.character) %>% 
  ungroup() %>% 
  # group_by(SA2_MAIN16) %>% arrange(date) %>% mutate(cumulative = cumsum(revenue)) %>% 
  # ungroup() %>% 
  arrange(SA2_MAIN16, property_type, date) %>% 
...
rm(temp, add, dates_min, dates_max, dates_sequence, rolling)
gc()
```
...
```{r, include=FALSE}
airbnb_sa2_revenue %<>% 
  mutate(ind_mis = ifelse(is.na(adr_50p), 1, NA)) %>% 
  group_by(SA2_MAIN16, property_type) %>% 
  mutate(sum_mis = sum(ind_mis, na.rm = TRUE)) %>% 
  ungroup() %>% 
  arrange(SA2_MAIN16, property_type, date) %>% 
  group_by(SA2_MAIN16, property_type, 
           grp = with(rle(ind_mis), rep(seq_along(lengths), lengths))
  ) %>%
  mutate(cum_mis = seq_along(grp)) %>%
  ungroup() %>%
  select(-grp, -ind_mis) %>% 
...
Not all SA2s have information for all months. `r comma(nrow(filter(airbnb_sa2_revenue, is.na(adr_50p))))` of the records (`r percent(nrow(filter(airbnb_sa2_revenue, is.na(adr_50p)))/nrow(airbnb_sa2_revenue))`) have missing information on monthly median ADR. 
...
```{r include=FALSE}
# time series length should be
# length(unique(airbnb_sa2_revenue$date))

# airbnb_H_mis <-
#   airbnb_sa2_revenue %>%
#   filter(property_type == "House") %>%
#   filter(sum_mis == length(unique(airbnb_sa2_revenue$date)))
...
airbnb_mis <- 
  SA2_H_include %>% 
  filter(SA2_MAIN16 %in% unique(airbnb_H_mis$SA2_MAIN16))
```

There are `r comma(nrow(airbnb_mis))` SA2 areas that do not have any Airbnb data for houses
...
APM_sa2 %<>% 
  filter(! (property_type == "House" & 
              SA2_MAIN16 %in% unique(airbnb_H_mis$SA2_MAIN16)))
```
...
airbnb_mis <- 
  SA2 %>% 
  filter(SA2_MAIN16 %in% unique(airbnb_U_mis$SA2_MAIN16))
```

There are `r comma(nrow(airbnb_mis))` SA2 areas that do not have any Airbnb data for units
...
APM_sa2 %<>% 
  filter(! (property_type == "Unit" & 
              SA2_MAIN16 %in% unique(airbnb_U_mis$SA2_MAIN16)))
```
...
rm(airbnb_mis, airbnb_H_mis, airbnb_U_mis)
gc()
```
...
There are `r comma(nrow(airbnb_mis))` SA2 areas that have missing **12 or more** months of Airbnb data **or** have **6 or more** consecutive months of Airbnb data missing for houses
...
There are `r comma(nrow(airbnb_mis))` SA2 areas that have missing **12 or more** months of Airbnb data **or** have **6 or more** consecutive months of Airbnb data missing for units
...
There are `r comma(nrow(airbnb_mis))` SA2 areas that have some missing Airbnb data for houses
...
There are `r comma(nrow(airbnb_mis))` SA2 areas that have some missing Airbnb data for units
...
airbnb_sa2_revenue %<>% 
  ...
  group_by(SA2_MAIN16, property_type) %>% 
  mutate(
    n_listings_int = as.integer(round(na_interpolation(n_listings, option = "stine"))),
    adr_25p_int = na_interpolation(adr_25p, option = "stine"),
    adr_50p_int = na_interpolation(adr_50p, option = "stine"),
    adr_75p_int = na_interpolation(adr_75p, option = "stine")
  ) %>% 
  ungroup()
...
# airbnb_sa2_revenue %<>% 
#   group_by(SA2_MAIN16, property_type) %>% 
#   mutate(adr_50p_int = as.integer(round(na_kalman(adr_50p, model = "auto.arima"))),
#          adr_25p_int = as.integer(round(na_kalman(adr_25p, model = "auto.arima"))),
#          adr_75p_int = as.integer(round(na_kalman(adr_75p, model = "auto.arima"))))

# # series is not periodic or has less than two periods
# airbnb_sa2_revenue %<>% 
#   group_by(SA2_MAIN16, property_type) %>% 
#   mutate(adr_50p_int = as.integer(round(na_seadec(adr_50p, find_frequency=TRUE))),
#          adr_25p_int = as.integer(round(na_seadec(adr_25p, find_frequency=TRUE))),
#          adr_75p_int = as.integer(round(na_seadec(adr_75p, find_frequency=TRUE))))
...
```{r warning=FALSE, fig.width=6, fig.height=3, out.width="600px", out.height="300px"}
airbnb_sa2_revenue %>% 
  filter(SA2_NAME16 == "Ashcroft - Busby - Miller", property_type == "House") %>% 
  ggplot(aes(x = date)) +
  # geom_point(aes(y = adr_50p_int)) +
  geom_point(aes(y = adr_50p, size = n_listings), col = "red", alpha = .5) +
  geom_line(aes(y = adr_50p_int)) +
  theme_minimal() + 
  labs(x = "", y = "Median ADR", title = "ADR - houses in Ashcroft - Busby - Miller")
```

```{r warning=FALSE, fig.width=6, fig.height=3, out.width="600px", out.height="300px"}
airbnb_sa2_revenue %>% 
  filter(SA2_NAME16 == "Derby - West Kimberley", property_type == "House") %>% 
  ggplot(aes(x = date)) +
  # geom_point(aes(y = adr_50p_int)) +
  geom_point(aes(y = adr_50p, size = n_listings), col = "red", alpha = .5) +
  geom_line(aes(y = adr_50p_int)) +
  theme_minimal() + 
  labs(x = "", y = "Median ADR", title = "ADR - units in Derby - West Kimberley")
```

```{r warning=FALSE, fig.width=6, fig.height=3, out.width="600px", out.height="300px"}
airbnb_sa2_revenue %>% 
  filter(SA2_NAME16 == "Edmonton", property_type == "Unit") %>% 
  ggplot(aes(x = date)) +
  # geom_point(aes(y = adr_50p_int)) +
  geom_point(aes(y = adr_50p, size = n_listings), col = "red", alpha = .5) +
  geom_line(aes(y = adr_50p_int)) +
  theme_minimal() + 
  labs(x = "", y = "Median ADR", title = "ADR - units in Edmonton")
```
...
SA2_centr %>% 
  left_join(filter(airbnb_sa2_revenue, 
                   date == ymd("2016-12-01") | date == ymd("2019-02-01"))) %>% 
  filter(property_type == "House") %>% 
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
  tm_dots(size = "adr_50p_int", col = "adr_50p_int", palette = "YlOrRd", n = 7, 
          id = "SA2_NAME16", popup.vars = c("SA2_NAME16", "adr_50p_int")) +
  tm_facets(by = "date", free.scales = FALSE)
```
...
SA2_centr %>% 
  ...
  filter(abs_supply == 0, supply_n > 0) %>% 
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
...
SA2_centr %>% 
  left_join(filter(airbnb_sa2_supply_days, 
                   reporting_month == ymd("2019-02-01"))) %>% 
  filter(abs_supply == 0 & supply_days > 0) %>% 
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
...
SA2_centr %>% 
  left_join(filter(airbnb_sa2_demand_days, 
                   reporting_month == ymd("2019-02-01"))) %>% 
  filter(abs_supply == 0 & demand_days > 0) %>% 
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
...
airbnb_sa2_revenue %<>%
  rename(airbnb_sum_mis = sum_mis, airbnb_cum_mis = cum_mis)
  
APM_sa2 %<>%
  rename(apm_sum_mis = sum_mis, apm_cum_mis = cum_mis) %>%
  # left_join(
  # 
  #   select(airbnb_sa2_supply_n,
  #          SA2_MAIN16, property_type, reporting_month,
  ...
  left_join(
    ...
    select(airbnb_sa2_revenue,
           SA2_MAIN16, property_type, date,
           n_months, n_listings, adr_25p, adr_50p, adr_75p,
           airbnb_sum_mis, airbnb_cum_mis,
           n_listings_int, adr_25p_int, adr_50p_int, adr_75p_int)
  ) %>%
  arrange(SA2_MAIN16, property_type, date)
...
saveRDS(SA2_U_include, "data/APM/clean/SA2_U_include.Rds")
saveRDS(SA2_U_centr_include, "data/APM/clean/SA2_U_centr_include.Rds")
saveRDS(SA2_H_include, "data/APM/clean/SA2_H_include.Rds")
saveRDS(SA2_H_centr_include, "data/APM/clean/SA2_H_centr_include.Rds")
