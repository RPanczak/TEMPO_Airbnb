...
```{r include=FALSE}
SA2_raw <- st_read("./data/geo/1270055001_sa2_2016_aust_shape/SA2_2016_AUST.shp", stringsAsFactors = FALSE)

minus_mig <- nrow(filter(SA2_raw, grepl("Migratory - Offshore - Shipping", SA2_NAME16)))
minus_add <- nrow(filter(SA2_raw, grepl("No usual address", SA2_NAME16)))
minus_rem <-  nrow(filter(SA2_raw, SA2_5DIG16 %in% c("91001", "91002", "91004", "11161")))

nrow(SA2_raw) - minus_mig - minus_add - minus_rem
```

There are `r comma(nrow(SA2_raw))` SA2 areas in Australia. We excluded areas with no physical location (including `r comma(nrow(minus_mig))` `Migratory–Offshore–Shipping` and `r comma(nrow(minus_add))` `No usual address` codes for each State and Territory). And `r comma(nrow(minus_rem))` remote areas: 
...
There are `r comma(nrow(APM_mis))` SA2 areas that do not have APM data for houses
...
There are `r comma(nrow(APM_mis))` SA2 areas that do not have APM data for units
...
Data from `2018-12-01` are kept (*based on 10 obs only!*)
...
Data imputed using Stineman interpolation.

```{r include=FALSE}
APM_sa2 %<>% 
  group_by(SA2_MAIN16, property_type) %>% 
  mutate(rent_50p_int = as.integer(round(na_interpolation(rent_50p, option = "stine"))),
...
Next, `r format(nrow(zero_aud), nsmall=0, big.mark=",")` monthly records of `r comma(length(unique(zero_aud$property_id)))` properties that had zero AUD income but some information in USD had income recalculated using 1.4 exchange rate.

Next, `r format(nrow(mis_aud), nsmall=0, big.mark=",")` monthly records of `r comma(length(unique(mis_aud$property_id)))` properties that had missing AUD income but some information in USD had income recalculated using 1.4 exchange rate.
...
monthly records of `r comma(length(unique(monthly_new_prep_revenue$property_id)))` properties in `r comma(length(unique(monthly_new_prep_revenue$SA2_MAIN16)))` SA2 areas. 
...
None of this areas have any info in APM data so it shouldnt be a problem.
...
Only including listing type `Entire home/apt` (ie. excluding `listing_type` "Shared room" & "Private room").
