...
ham_select <- readRDS("./data/ha/clean/ham_select.Rds") %>% 
  dplyr::filter(reporting_month >= ymd("2017-07-01")) %>% 
...
# saveRDS(monthly_prep_gno, file = "./data/ha/clean/monthly_prep_gno.rds")

rm(remote_monthly, ham_select, hap_select) #, max_guests_1, max_guests_2)
gc()
```

Final dataset of `r format(nrow(monthly_prep_gno), nsmall=0, big.mark=",")`
monthly records of `r format(length(unique(monthly_prep_gno$property_id)), nsmall=0, big.mark=",")` properties was aggregated to SA3 levels. 
...
```{r}
SA3_na <- anti_join(SA3, select(monthly_prep_gno, SA3_CODE16))
```
...
frq(monthly_prep_gno, property_type, sort.frq = "desc")
```
...
stopifnot(nrow(SA3) * length(unique(monthly_prep_gno$reporting_month)) == nrow(ha_sa3))
stopifnot(ha_sa3$max_gno <= ha_sa3$max_gna)
```
...
ggplot(data = ha_sa3, 
  ...
  theme_minimal() + xlab("") + ylab("HomeAway GNO [*1,000]") +
...
ggplot(data = ha_sa3, 
  ...
  geom_line(colour = "darkorchid4", alpha=0.5) +
  facet_wrap(vars(STE_SHORT)) +
  theme_minimal() + xlab("") + ylab("HomeAway listings [*1,000]") +
  scale_x_date(minor_breaks = "2 months")
```
...
ggplot(data = ha_sa3, 
  ...
  geom_line(colour = "darkorchid4", alpha=0.5) +
  facet_wrap(vars(STE_SHORT)) +
  theme_minimal() + xlab("") + ylab("HomeAway bedrooms [*1,000]") +
  scale_x_date(minor_breaks = "2 months")
```
...
ggplot(data = ha_sa3, 
  ...
  geom_line(colour = "darkorchid4", alpha=0.5) +
  facet_wrap(vars(STE_SHORT)) +
  theme_minimal() + xlab("") + ylab("HomeAway supply nights [*1,000]") +
  scale_x_date(minor_breaks = "2 months")
```
...
ggplot(ha_sa3_18, aes(y = max_gno/1000, x = nights/1000)) + 
  geom_smooth(method = lm, se = FALSE) +
  geom_point(aes(colour = STE_SHORT), alpha = 0.5) + 
  geom_rug() +
  theme_minimal() + ylab("GNO") + xlab("HomeAway reservation days") +
...
ggplot(ha_sa3_18, aes(y = max_gno/1000, x = reservations/1000)) + 
  ...
  geom_point(aes(colour = STE_SHORT), alpha = 0.5) + 
  geom_rug() +
  theme_minimal() + ylab("GNO") + xlab("HomeAway reservations") +
  facet_wrap(~reporting_month)
```
...
ggplot(data = regions, 
  ...
  geom_col() +
  theme_minimal() + xlab("") + ylab("HomeAway yearly GNO") +
...
SA3 %>% 
  left_join(filter(ha_sa3_prop_18,
