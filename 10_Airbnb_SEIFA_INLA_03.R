...
# ########################################
# ####
f1 = cumulative ~ f(time, model = "rw2", hyper = lgprior2) + 
...
# saveRDS(m1, file = "./res/INLA/03/m1.rds")

m1 <- readRDS(file = "./res/INLA/03/m1.rds")
...
plot(m1$summary.fitted.values$mean, airbnb_sa2$cumulative)
...
# ####
f2 = cumulative ~ f(time, model = "rw2", hyper = lgprior2) + 
  f(SA2_MAIN16, model="iid", param = prior.nu) + 
  as.factor(IRSD_d)
...
# saveRDS(m2, file = "./res/INLA/03/m2.rds")

m2 <- readRDS(file = "./res/INLA/03/m2.rds")
...
plot(m2$summary.fitted.values$mean, airbnb_sa2$cumulative)
...
f3 = cumulative ~ 
  f(time,   IRSD_d_1,  model = "rw2", hyper = lgprior2) + 
  f(time2,  IRSD_d_2,  model = "rw2", hyper = lgprior2) + 
  f(time3,  IRSD_d_3,  model = "rw2", hyper = lgprior2) + 
  f(time4,  IRSD_d_4,  model = "rw2", hyper = lgprior2) + 
  f(time5,  IRSD_d_5,  model = "rw2", hyper = lgprior2) + 
  f(time6,  IRSD_d_6,  model = "rw2", hyper = lgprior2) + 
  f(time7,  IRSD_d_7,  model = "rw2", hyper = lgprior2) + 
  f(time8,  IRSD_d_8,  model = "rw2", hyper = lgprior2) + 
  f(time9,  IRSD_d_9,  model = "rw2", hyper = lgprior2) + 
  f(time10, IRSD_d_10, model = "rw2", hyper = lgprior2) + 
  f(SA2_MAIN16, model="iid", param = prior.nu) + 
  as.factor(IRSD_d)
...
# saveRDS(m3, file = "./res/INLA/03/m3.rds")

m3 <- readRDS(file = "./res/INLA/03/m3.rds")
...
plot(m3$summary.fitted.values$mean, airbnb_sa2$cumulative)
