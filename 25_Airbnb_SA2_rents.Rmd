---
title: "Airbnb"
subtitle: "Rents & income on SA2 level"
...
Data from `2018-12-01` were removed (*based on 10 obs only!*)
...
```{r warning=FALSE}
APM_sa2 %>% 
  filter(SA2_NAME16 == "Warnbro", property_type == "Unit") %>% 
  ggplot(aes(x = date)) +
  geom_line(aes(y = rent_50p), size = 2, col = "red") +
  geom_line(aes(y = rent_50p_int)) +
  theme_minimal() + 
  labs(x = "", y = "Median rent", title = "Rent - units in Warnbro")
```
