...
airbnb_sa4_12mo_wide$cluster7 <- getClusters(cld_airbnb, 7)
...
airbnb_sa4_12mo %>% 
  mutate(month = strftime(reporting_month, "%m")) %>% 
  ggplot(aes(x=month, y=revenue_rel)) +
  geom_line(aes(color=cluster5, group=SA4_CODE16), alpha=0.20) +
...
tm_shape(SA4) +
  tm_polygons("cluster5", border.col = "white", lwd = 0.5, 
              id = "SA4_NAME16", popup.vars = c("cluster5"), 
              palette = "Dark2") +
  tm_shape(STE) +
  tm_borders(lwd = 0.5, alpha = 0.5)  
...
airbnb_sa4_12mo %>% 
  mutate(month = strftime(reporting_month, "%m")) %>% 
  ggplot(aes(x=month, y=revenue_rel)) +
  # geom_line(aes(color=cluster7, group=SA4_CODE16), alpha=0.20) +
  geom_smooth(aes(group=cluster7, color=cluster7), size = 1, se = FALSE) +
  theme_minimal() + xlab("") + ylab("Airbnb relative revenue {%}") +
  scale_colour_brewer(palette = "Dark2") 
...
tm_shape(SA4) +
  tm_polygons("cluster7", border.col = "white", lwd = 0.5, 
              id = "SA4_NAME16", popup.vars = c("cluster7"), 
              palette = "Dark2") +
  tm_shape(STE) +
  tm_borders(lwd = 0.5, alpha = 0.5)  
```
...
airbnb_ste <- airbnb_sa4 %>% 
  ...
  summarise(revenue = sum(revenue)) %>% 
  ungroup() %>% 
  group_by(State_short, cluster5) %>% arrange(reporting_month) %>% mutate(cumulative = cumsum(revenue)) %>% 
  ungroup() %>% 
