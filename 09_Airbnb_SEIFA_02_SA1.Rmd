...
# sum_up(zero_inc, revenue_native)
# sum_up(zero_inc, revenue_usd)
...
rm(monthly_select, property_select, temp)
gc()
```
...
`r format(nrow(filter(SA1, !SA1_MAIN16 %in% unique(airbnb_monthly$SA1_MAIN16))), nsmall=0, big.mark=",")` areas out of `r format(nrow(SA1), nsmall=0, big.mark=",")` SA1s included have no Airbnbs inside. These areas will have $0 Airbnb income throught the study period.

```{r}
# temp <- SA1 %>% filter(!SA1_MAIN16 %in% unique(airbnb_monthly$SA1_MAIN16))
# 
# tm_shape(temp) +
...
`r format(nrow(filter(SA1, SA1_MAIN16 %in% unique(airbnb_monthly$SA1_MAIN16))), nsmall=0, big.mark=",")` areas out of `r format(nrow(SA1), nsmall=0, big.mark=",")` SA1s included have Airbnbs inside. These areas had their Airbnb income calculated per month as cumulative sum of all income from all propoerties inside area.
...
stopifnot(nrow(SA1) * length(unique(airbnb_monthly$reporting_month)) == nrow(airbnb_sa1))
rm(temp)
gc()
```
...
```{r}
filter(airbnb_sa1, reporting_month == as.Date("2017-10-01")) %>% 
  frq(IRSD_d)
```
...
coast_near <- left_join(coast_near, SA1_2016_join) %>% 
  group_by(SA1_MAIN16) %>% 
...
temp <- filter(airbnb_sa1, reporting_month == as.Date("2017-10-01"))
...
Amount of dollars from Airbnb till 2017-10-01:
...
left_join(SA1, filter(airbnb_sa1, reporting_month == as.Date("2017-10-01"))) %>% 
  tm_shape() + 
