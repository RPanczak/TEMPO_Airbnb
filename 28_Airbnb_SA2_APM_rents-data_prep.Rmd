...
Monthly aggregated data with monthly medians of income:
...
SA2_centr %>% 
  ...
  select(-SA2_MAIN16, -SA2_5DIG16, -starts_with("SA4"), -starts_with("STE")) %>% 
  tm_shape() +
  tm_dots(size = "adr_50p", col = "adr_50p", palette = "YlOrRd", n = 7, 
          id = "SA2_NAME16", popup.vars = c("SA2_NAME16", "adr_50p")) +
...
APM_sa2 %<>%
  ...
  left_join(
    ...
    select(airbnb_sa2_revenue, 
           ...
           n_months, n_listings, revenue_med, adr_25p, adr_50p, adr_75p)
  ) %>% 
  arrange(SA2_MAIN16, property_type, date)
