...
# TR 2015 outlines 
TR15 <- st_read("data/geo/1270055003_tr_2015_aust_shape/TR_2015_AUST.shp", stringsAsFactors = FALSE)
class(TR15); nrow(TR15); names(TR15)
# View(TR15)
st_crs(TR15)
TR15$AREA_SQKM <- NULL
...
# table(duplicated(TR15$TR_NAME15))

saveRDS(TR15, file = "data/geo/TR_2015_AUST.rds")

# TR 2016 outlines 
TR16 <- st_read("data/geo/1270055003_tr_2016_aust_shape/TR_2016_AUST.shp", stringsAsFactors = FALSE)
class(TR16); nrow(TR16); names(TR16)
# View(TR16)
st_crs(TR16)
TR16$AREASQKM16 <- NULL

TR16 <- ms_simplify(TR16, keep = 0.05, weighting = 0.7) # default settings
# qtm(TR16)

# table(duplicated(TR16$TR_NAME16))

saveRDS(TR16, file = "data/geo/TR_2016_AUST.rds")

# SA3 outlines
SA3 <- st_read("./data/geo/1270055001_sa3_2016_aust_shape/SA3_2016_AUST.shp", stringsAsFactors = FALSE)
# View(SA3)
# st_crs(SA3)
# qtm(SA3)
saveRDS(SA3, file = "./data/geo/SA3_2016_AUST.rds")

# removing other teritories and zeroes
SA3 <- 
  SA3 %>% 
...
SA3_centr <- read_csv("data/geo/SA3_2016_pop_center_median.csv") %>% 
  select(-OBJECTID) %>% 
  rename(lon = XCoord, 
         lat = YCoord) %>% 
  mutate(SA3_CODE16 = as.character(SA3_CODE16)) %>% 
  right_join(select(st_drop_geometry(SA3), SA3_CODE16)) %>% 
...
```{r joins_prep, eval=FALSE, include=FALSE}
SA1_2016_join_new <- read_csv("./data/airdna/raw/Spatial_join/SA1_2016_join_new.zip") %>% 
  dplyr::rename(property_id = PROPERTY_ID) %>% 
  select(-XCoord, -YCoord, -JOIN_COUNT)

saveRDS(SA1_2016_join_new, file = "./data/airdna/clean/SA1_2016_join_new.rds")

POA_2016_join_new <- read_csv("./data/airdna/raw/Spatial_join/POA_2016_join_new.zip") %>% 
...
SA1_2016_join_new <- readRDS("./data/airdna/clean/SA1_2016_join_new.rds") %>% 
  dplyr::select(property_id, starts_with("SA1"), starts_with("SA2"), 
                starts_with("SA3"), starts_with("SA4"), 
                starts_with("STE"))
```
...
This is an overview of second **raw** `Airdna` dataset. 
...
```{r, include=FALSE}
property_new_raw <- read_csv("./data/airdna/raw/Australia_Airbnb_property_2019-02-01.zip")
...
# table(is.na(SA1_2016_join_new$SA1_MAIN16))
# table(is.na(property_new$SA1_MAIN16))

# temp <- property_new %>% 
#   filter(is.na(SA1_MAIN16))

rm(property_new_raw, SA1_2016_join_new) #, temp)
gc()
...
property_new_select <- property_new %>% 
  select(property_id, host_id, 
         ...
         bedrooms, bathrooms, max_guests, listing_url)
...
```{r, include=FALSE}
monthly_new_raw <- read_csv("./data/airdna/raw/Australia_Airbnb_Monthly_2019-03-22.zip")
...
monthly_new %<>% left_join(SA1_2016_join_new)

# table(is.na(SA1_2016_join_new$SA1_MAIN16))
# table(is.na(monthly_new$SA1_MAIN16))

rm(monthly_new_raw, SA1_2016_join_new)
gc()
```
...
monthly_new_select <- monthly_new %>% 
  select(property_id, reporting_month, active, 
         listing_type, property_type, bedrooms, 
         number_of_reservations, occupancy_rate, reservation_days, 
         available_days, blocked_days,
         revenue_usd, revenue_native, adr_usd, adr_native,
         starts_with("SA"), starts_with("STE"))
