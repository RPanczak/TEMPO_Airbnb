...
Nine very high monthly values were recalculated from more realistic ADR indicators.
...
Final dataset consists of `r format(nrow(airbnb_new_monthly), nsmall=0, big.mark=",")`
monthly records of `r format(length(unique(airbnb_new_monthly$property_id)), nsmall=0, big.mark=",")` properties. 
...
FInal geographical set consists of `r nrow(SA2)` SA2s. 
...
There are `r number(nrow(airbnb_SEIFA_mis))` records of `r number(length(unique(airbnb_SEIFA_mis$property_id)))` propoerties in these areas. 
...
rm(airbnb_SEIFA_na, airbnb_SEIFA_mis)
gc()
```
