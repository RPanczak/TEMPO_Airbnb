...
# messaging file
fileConn <- file("/RDS/Q0786/data/Twitter/clean/tweets_proc_2019_03_18.log", "w")
...
# Cleand empty rows 
tweets_proc_2019_03_18 <- tweets_raw[!apply(is.na(tweets_raw) | tweets_raw == "", 1, all), ]
# Cleand duplis
tweets_proc_2019_03_18 <- tweets_proc_2019_03_18[!duplicated(tweets_proc_2019_03_18), ]

rm(tweets_raw)
gc()

### Rename variables
tweets_proc_2019_03_18 %<>% 
...
# length(unique(tweets_proc_2019_03_18$user_id_str))
# length(unique(tweets_proc_2019_03_18$user_screen_name))

tweets_proc_2019_03_18 %<>% 
  mutate(UID = group_indices_(tweets_proc_2019_03_18, .dots=c("user_id_str", "user_screen_name")))

#### Fixing tweet id 

# isUnique(tweets_proc_2019_03_18$id_str)
# length(tweets_proc_2019_03_18$id_str)
# length(unique(tweets_proc_2019_03_18$id_str))

# tweets_proc_2019_03_18 %>% select(id_str, created_at, place_full_name) %>% filter(id_str == "1.00028e+18")
...
# summarytools::freq(tweets_proc_2019_03_18$t4, order = "freq")#, report.nas = TRUE) 
...
# summary(tweets_proc_2019_03_18$created_at)
...
# place_full_name <- tweets_proc_2019_03_18 %>% 
#   filter(is.na(lat)) %>% 
...
complete_place_full_name <- tweets_proc_2019_03_18 %>% 
  filter(is.na(lat)) %>% 
...
complete_tweets_geo_tz <- tweets_proc_2019_03_18 %>% 
  group_by(lat, lon) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
  filter(!is.na(lat)) %>% 
...
saveRDS(tweets_proc_2019_03_18, file = "/RDS/Q0786/data/Twitter/clean/tweets_proc_2019_03_18.rds")
