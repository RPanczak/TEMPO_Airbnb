...
Data are openly available. Unfortunately they can only be dowloaded in form of horribly designed spreadsheets. Note the empty cells! Comment for them says:
...
SA2 <- left_join(SA2, filled, by = c("SA2_NAME16" = "SA2"))
...
SA2 <- left_join(SA2, zeroes, by = c("SA2_NAME16" = "SA2"))
SA2$zeroes[is.na(SA2$zeroes)] <- 0
table(SA2$zeroes)

table(SA2$filled, SA2$zeroes)
...
s <- seq(
  from = as.Date("2015-07-01", format = "%Y-%m-%d"),
...
for (i in seq_along(s)) {
  myMap <- ggplot() +
    geom_point(
      data = filter(temp, as.Date(date, format = "%Y-%m-%d") == s[[i]]),
